...
class OPUSSearchObservationsInput(InputSchema):
    ...
    limit: int = Field(default=100, description="Maximum observations to return")
    startobs: int = Field(default=1, description="Starting observation index for pagination")
    order: str = Field(default="time1,opusid", description="Sort order (e.g., 'time1,opusid')")
