...
class OPUSClient:
    ...
    def __init__(
        ...
        """
        self.base_url = base_url or os.getenv("OPUS_BASE_URL", self.BASE_URL)
        self.timeout = timeout if timeout is not None else float(os.getenv("OPUS_TIMEOUT", str(self.DEFAULT_TIMEOUT)))
        self.max_retries = max_retries if max_retries is not None else int(os.getenv("OPUS_MAX_RETRIES", "3"))
